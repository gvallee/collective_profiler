...
{
    DEBUG_ALLTOALL_PROFILING("Adding pattern\n");
    if (patterns == NULL)
    {
        DEBUG_ALLTOALL_PROFILING("Adding pattern to empty list\n");
        return new_pattern_with_size(num_ranks, num_peers, size);
    }
    else
    {
        avPattern_t *ptr = patterns;
        DEBUG_ALLTOALL_PROFILING("We already have patterns, comparing...\n");

        while (ptr != NULL)
        {
            if (ptr->n_ranks == num_ranks && ptr->n_peers == num_peers && size == ptr->comm_size)
            {
                DEBUG_ALLTOALL_PROFILING("Pattern already exists\n");
                ptr->n_calls++;
                DEBUG_ALLTOALL_PROFILING("Pattern successfully added\n");
                return patterns;
            }
            ptr = ptr->next;
        }

        DEBUG_ALLTOALL_PROFILING("Adding new pattern to list\n");
        ptr = patterns;
        assert(ptr);
        while (ptr->next != NULL)
        {
...
avCallPattern_t *extract_call_patterns(int callID, int *send_counts, int *recv_counts, int size)
{
    ...
    DEBUG_ALLTOALL_PROFILING("Extracting call patterns\n");
    ...
    DEBUG_ALLTOALL_PROFILING("Handling call send patterns\n");
    for (i = 0; i < size; i++)
    {
    ...
    DEBUG_ALLTOALL_PROFILING("Handling call receive patterns\n");
    for (i = 0; i < size; i++)
    {
