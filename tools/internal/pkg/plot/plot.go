...
func generateCallDataFiles(dir string, outputDir string, hostMap map[string][]int, sendHeatMap map[int]int, recvHeatMap map[int]int, execTimeMap map[int]float64, lateArrivalMap map[int]float64) (string, error) {
	...
	sort.Ints(a)
	gnuplotScript, err := generatePlotScript(outputDir, numRanks, maxValue, a, hosts)
	if err != nil {
		return "", err
	}
...
func generatePlotScript(outputDir string, numRanks int, maxValue int, values []int, hosts []string) (string, error) {
	plotScriptFile := filepath.Join(outputDir, "profiler.gnuplot")
	...
	str := "plot "
	for _, hostname := range hosts {
		str += "\"ranks_" + hostname + ".txt\" using 0:1 with boxes title '" + hostname + "', \\ \n"
	}
	...
	str += fmt.Sprintf("\"%s.txt\" using 4 with points ls 3 title \"execution time (ms)\", \\ \n", hosts[0])
	str += fmt.Sprintf("\"%s.txt\" using 5 with points ls 4 title \"late arrival timing (ms)\", \\ \n", hosts[0])
	str += fmt.Sprintf("\"%s.txt\" using 6 with points ls 5 title \"bandwidth (MB/s)\", \\ \n", hosts[0])
...
func Create(dir string, outputDir string, hostMap map[string][]int, sendHeatMap map[int]int, recvHeatMap map[int]int, execTimeMap map[int]float64, lateArrivalMap map[int]float64) error {
	gnuplotScript, err := generateCallDataFiles(dir, outputDir, hostMap, sendHeatMap, recvHeatMap, execTimeMap, lateArrivalMap)
	if err != nil {
		return err
	}
	...
	cmd := exec.Command(gnuplotBin, "<", gnuplotScript)
	cmd.Dir = outputDir
	err = cmd.Run()
	if err != nil {
		return err
