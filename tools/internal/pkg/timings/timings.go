...
func DetectTimingsFiles(dir string) (map[string]*CollectiveTimings, error) {
	...
	for _, file := range f {
		...
		if indexFirstDelimiter == -1 {
			return nil, fmt.Errorf("timing data file does not follow format: %s", filename)
		}
...
func HandleTimingFiles(codeBaseDir string, dir string, totalNumCalls int) (map[string]*CollectiveTimings, map[int]float64, map[int]float64, error) {
	...
	data, err := DetectTimingsFiles(dir)
	if err != nil {
		return nil, nil, nil, err
	}
