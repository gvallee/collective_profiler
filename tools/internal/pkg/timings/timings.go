...
func PreprocessRawDataFile(filePath string, outputDir string, a2aTimes map[int]map[int]float64, lateArrivalTimes map[int]map[int]float64) error {
...
func extractTimingsFromSeparateTimingFile(file string, callsMaps *maps.CallsDataT, callData map[int]map[int]float64) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
	...
	for {
		...
		callData[callID] = make(map[int]float64)
		...
		for {
			...
			commWorldRank := callsMaps.RanksMap[callID][commRank]
			callData[callID][commWorldRank] = value
		}
	}
...
func analyzeTimingsFiles(dir string, files []string, callsMaps *maps.CallsDataT) (map[int]map[int]float64, map[int]map[int]float64, error) {
	// call-centric maps where the key is the callID and the value an array of time, the index of the array being the rank on the communciator
	...
	case separateTimingsFiles:
		for _, file := range files {
			if strings.HasPrefix(filepath.Base(file), AlltoallFilePrefix) {
				err := extractTimingsFromSeparateTimingFile(file, callsMaps, a2aTimes)
				if err != nil {
					return nil, nil, err
				}
			}
			...
			if strings.HasPrefix(filepath.Base(file), LateArrivalFilePrefix) {
				err := extractTimingsFromSeparateTimingFile(file, callsMaps, lateArrivalTimes)
				if err != nil {
					return nil, nil, err
				}
			}
...
func HandleTimingFiles(dir string, callsMaps *maps.CallsDataT) (map[int]map[int]float64, map[int]map[int]float64, error) {
	...
	return analyzeTimingsFiles(dir, files, callsMaps)
}
