...
type SingleTimingFileInfoT struct {
	// Path the single tining file
	Path string
}

type CoupledTimingFilesInfoT struct {
	A2AExecTimingFiles     []string
	LateArrivalTimingFiles []string
}

type ProfileFilesT struct {
	RawCounts          RawCountsFilesInfoT
	SingleTimingFile   SingleTimingFileInfoT
	CoupledTimingFiles CoupledTimingFilesInfoT
}

func (info *ProfileFilesT) IncludesSingleTimingsFile() bool {
	if info.SingleTimingFile.Path != "" {
		return true
	}
	return false
}
...
func (info *ProfileFilesT) IncludesCoupledTimingFiles() bool {
	if len(info.CoupledTimingFiles.A2AExecTimingFiles) > 0 || len(info.CoupledTimingFiles.LateArrivalTimingFiles) > 0 {
		return true
	}
	return false
}
